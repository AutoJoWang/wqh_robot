...
class Params:
    ...
    # slip compensation gains (u_comp = [k_v * d_x_local, k_omega * d_theta + k_y * d_y_local])
    slip_k_v: float = 0.25
    slip_k_omega: float = 0.20
    slip_k_y: float = 0.18
...
def mpc_plan(
    ...
    *,
    ...
    Args:
        ...
        slip_linear: 纵向滑移衰减系数 (0,1]，实际前进速度 = slip_linear * v_cmd
        slip_compensate: 是否启用滑移补偿。True 时采用“运动学残差后验补偿”：
            计算残差并投影到车体坐标后，生成 delta_u 叠加到 MPC 控制量
    ...
    for _ in range(int(param.max_iteration)):
        ...
        if shouldRotateToGoal((robot.x, robot.y), goal, param):
            ...
            if shouldRotateToPath(abs(e_theta), math.pi / 4.0, param):
                ...
                u_mpc, u_p, du = mpcControl(s, s_d, u_r, u_p, robot, param)
                u = u_mpc
                if slip_compensate:
                    delta_u = slip_compensation_delta(
                        robot=robot,
                        s=s,
                        s_d=s_d,
                        u_r=u_r,
                    ...
                    u = np.array(
                        [
                            linearRegularization(robot, float(u_mpc[0] + delta_u[0]), param),
                            angularRegularization(robot, float(u_mpc[1] + delta_u[1]), param),
                        ],
                        dtype=float,
                    )
...
def f(
    ...
    *,
    ...
    v_cmd = float(u[0])
    w = float(u[1])
    slip = float(slip_linear)
    if slip <= 0.0 or slip > 1.0:
        raise ValueError("slip_linear must be in (0, 1]")

    # 滑移模型: 实际速度 = slip * v_cmd
    # 注意: 补偿不在动力学内“除以 slip”，而是在 mpc_plan 中通过 delta_u 叠加完成
    _ = slip_compensate  # kept for backward-compatible signature
    v_eff = slip * v_cmd

    # MATLAB uses an augmented state and linear map; this is equivalent.
    x_new = robot.x + dt * math.cos(robot.theta) * v_eff
    y_new = robot.y + dt * math.sin(robot.theta) * v_eff
    theta_new = robot.theta + dt * w

    return RobotState(x=x_new, y=y_new, theta=theta_new, v=v_cmd, w=w)


def linearized_ab(u_r: np.ndarray, s_d: np.ndarray, dt: float) -> Tuple[np.ndarray, np.ndarray]:
    """与 mpcControl 一致的一步线性化矩阵 A, B。"""
    A = np.eye(3)
    A[0, 2] = -float(u_r[0]) * math.sin(float(s_d[2])) * float(dt)
    A[1, 2] = float(u_r[0]) * math.cos(float(s_d[2])) * float(dt)
    B = np.zeros((3, 2))
    B[0, 0] = math.cos(float(s_d[2])) * float(dt)
    B[1, 0] = math.sin(float(s_d[2])) * float(dt)
    B[2, 1] = float(dt)
    return A, B


def slip_compensation_delta(
    *,
    robot: RobotState,
    s: np.ndarray,
    s_d: np.ndarray,
    ...
    """
    运动学残差后验补偿:
      residual_global = tilde_xi_real - hat_tilde_xi
      residual_local  = R(theta_real)^T * residual_global
      u_comp = [k_v * d_x_local, k_omega * d_theta_local + k_y * d_y_local]
    """
    # ideal one-step predicted error under nominal model
    A, B = linearized_ab(u_r, s_d, dt)
    hat_tilde_xi = (A @ (s - s_d).reshape((3, 1)) + B @ np.asarray(du_qp).reshape((2, 1))).reshape((3,))

    # one-step "real" error under slip-attenuated plant
    robot_real = f(robot, u_mpc, dt, slip_linear=slip_linear, slip_compensate=False)
    xi_real = np.array([robot_real.x, robot_real.y, robot_real.theta], dtype=float)
    tilde_xi_real = np.array(
        [
            xi_real[0] - float(s_d[0]),
            xi_real[1] - float(s_d[1]),
            regularizeAngle(float(xi_real[2] - float(s_d[2]))),
        ],
        dtype=float,
    )
    ...
    # global residual
    residual_global = tilde_xi_real - hat_tilde_xi
    residual_global[2] = regularizeAngle(float(residual_global[2]))

    # project residual into robot local frame
    theta_real = float(xi_real[2])
    cos_t = math.cos(theta_real)
    sin_t = math.sin(theta_real)
    d_x_local = residual_global[0] * cos_t + residual_global[1] * sin_t
    d_y_local = -residual_global[0] * sin_t + residual_global[1] * cos_t
    d_theta_local = float(residual_global[2])

    # compensation from residual
    u_comp_v = float(param.slip_k_v) * d_x_local
    u_comp_omega = float(param.slip_k_omega) * d_theta_local + float(param.slip_k_y) * d_y_local
    return np.array([u_comp_v, u_comp_omega], dtype=float)
...
def mpcControl(
    ...
    # original state matrix A (3x3), control matrix B (3x2)
    A, B = linearized_ab(u_r, s_d, dt)
